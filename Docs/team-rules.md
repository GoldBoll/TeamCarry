...
## 협업 규칙 요약 (상세는 CONTRIBUTING.md)

- `main` 직접 커밋·push 금지 → `feat/*` 브랜치 + PR
- `.uasset`/`.umap`은 바이너리 → **동시 수정 금지**, 작업 전 디스코드 공유(사실상 잠금)
- 복제 변수 추가 시 신장식과 협의 (네트워크·물리는 전 역할 분산)
- 빌드 자동화로 머지마다 패키징 — 깨진 빌드 조기 발견
